...
bool S_PathToBoss::Start()
{
	scene_str = "Castle Bridge";
	...
	if (App->map->Load("path to castle.tmx") == true)
	{
		int w, h = 0;
		uchar* data = nullptr;
...
bool S_PathToBoss::Save(pugi::xml_node& node) const
{
	return true;
}

bool S_PathToBoss::Load(pugi::xml_node& node)
{
	return true;
}