...
bool S_OptionsVideo::Awake(pugi::xml_node& conf)
{
	...
	fullscreen = App->gui->CreateButton(iPoint(X_pos + offset, 200), &std::string(conf.child("fullscreen").attribute("value").as_string("Fullscreen")), ButtonType::idle_hover_pressed, &idle_button_rect, &hover_button_rect, &pressed_button_rect, false);
	fullscreen->SetFont(App->font->Sherwood20);
	((Gui*)fullscreen)->SetListener(this);
	fullscreen->SetVisible(false);
	fullscreen->Focusable(true);

	fullscreen_check = App->gui->CreateCheck(iPoint(X_pos + 275, 229), &idle_check_rect, &pressed_check_rect);
	((Gui*)fullscreen_check)->SetListener(this);
	fullscreen_check->SetVisible(false);

	vsync = App->gui->CreateButton(iPoint(X_pos + offset, 310), &std::string(conf.child("vsync").attribute("value").as_string("VSYNC")), ButtonType::idle_hover_pressed, &idle_button_rect, &hover_button_rect, &pressed_button_rect, false);
	vsync->SetFont(App->font->Sherwood20);
	((Gui*)vsync)->SetListener(this);
	vsync->SetVisible(false);
	vsync->Focusable(true);

	vsync_check = App->gui->CreateCheck(iPoint(X_pos + 275, 339), &idle_check_rect, &pressed_check_rect);
...
bool S_OptionsVideo::Save(pugi::xml_node& node) const
{
	return true;
}

bool S_OptionsVideo::Load(pugi::xml_node& node)
{
	return true;
}