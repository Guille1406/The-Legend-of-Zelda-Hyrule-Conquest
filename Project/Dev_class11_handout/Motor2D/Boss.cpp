...
void Boss::GetEvent()
{
	...
	if((dist_link < 300 || dist_zelda < 300) &&can_attack) {
		switch (actual_phase) {
		case boss_phase_1:
			if (dist_link <= 300 &&dist_link<dist_zelda && dist_zelda>120 && dist_link >120 && state != boss_attack_zelda)
...
void Boss::LaserAttack()
{
	...
	if (laser_charging_time.Read() > 2500) {
		//Create collider
		Collider* temp_collider = App->collision->AddCollider({ focus_point_before_attack.x-32,focus_point_before_attack.y - 32,64,64 }, collider_boss_explosion, this, App->enemy);
