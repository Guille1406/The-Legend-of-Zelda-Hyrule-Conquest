...
public:
	...
	void Action(Character* pulling_character) {
		...
		case direction::up:
			if (App->object->GetLogic(logic_height, { rect.x/16 ,rect.y/16 -1 }, true) == 0) {
				rect.y--;
			}
			break;
		case direction::down:
			if (App->object->GetLogic(logic_height, { rect.x / 16 ,rect.y / 16 +2 }, true) == 0) {
				rect.y++;
			}
			break;
		case direction::left:
			if (App->object->GetLogic(logic_height, { rect.x / 16 -1 ,rect.y / 16 }, false) ==0) {
				rect.x--;
			}
			break;
		case direction::right:
			if (App->object->GetLogic(logic_height, { rect.x / 16 + 2,rect.y / 16 }, false) == 0) {
				rect.x++;
			}
			break;
