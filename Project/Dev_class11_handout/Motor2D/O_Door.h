	...
	void Open() {
		...
		/*for (int i = 0; i < rect.w / 16; i++) {
			for (int n = 0; n < rect.h / 16; n++) {
				...
				temp.y = rect.y + n * 16;
				active = false;
				if(App->map->V_Colision[logic_height][0].data[(temp.y / 16) * App->map->data.height + temp.x / 16] == App->map->TILE_COL_ID)
				App->map->V_Colision[logic_height][0].data[(temp.y / 16) * App->map->data.height + temp.x / 16] = 0;
			}
		}*/
	};
