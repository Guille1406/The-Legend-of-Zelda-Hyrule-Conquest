...
#include "Particle.h"
/*
P_Firework::P_Firework(SceneElement* element, iPoint* object, iPoint position_static, SDL_Rect initial_rect, iPoint timelife_, fPoint speed_particle, Part_Direction p_direction, int num_particles, int num_textures, iPoint next_textture_, iPoint last_textures_)
{
	if (element != nullptr)
	{
	...
	if (particle.size() == 1)
	{
		if (particle[0]->isDead())
		{
			...
			for (int i = 1; i < number_particles; i++)
			{
				speed.x = 100;
				speed.y = 100;
				Particle* temp = new Particle(pos, iPoint(0, 0), timelife, speed, Part_Direction::Part_Direc_RANDOM_FIREWORK, i_rect, size_rect, n_textures, true, Wind::Part_Wind_NULL, next_textures);
				particle.push_back(temp);
			}
			timelife.y -= 1;
		}
		...
		for (int i = 1; i < number_particles; i++)
		{
			if (particle[i]->isDead() && particle[i]->GetRepeat())
			{
				...
				for (int k = 0; k < MULITFIREWORK; k++)
				{
					speed.x = 100;
					speed.y = 100;
					Particle* temp = new Particle(particle[i]->GetPosition(), iPoint(0, 0), timelife, speed, Part_Direction::Part_Direc_RANDOM_FIREWORK, i_rect, size_rect, n_textures, true, Wind::Part_Wind_NULL, last_textures);
					particle.push_back(temp);
