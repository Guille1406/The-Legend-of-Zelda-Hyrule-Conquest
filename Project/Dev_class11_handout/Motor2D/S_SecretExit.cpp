...
bool S_SecretExit::Start()
{
	...
	if (App->map->Load("secret exit.tmx") == true)
	{
		int w, h = 0;
		uchar* data = nullptr;
	...
	App->audio->PlayMusic("audio/music/Overworld_Theme.ogg", 0);
...
bool S_SecretExit::Save(pugi::xml_node& node) const
{
	return true;
}

bool S_SecretExit::Load(pugi::xml_node& node)
{
	return true;
}